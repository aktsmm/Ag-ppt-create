...
| `extract_shapes.py`   | python-pptx 公式ドキュメント            | テキスト抽出           |
| `apply_content.py`    | python-pptx 公式ドキュメント            | テキスト置換           |
| `gen_preview.py`      | python-pptx + Pillow ドキュメント       | サムネイル生成         |
| `convert_html.js`     | PptxGenJS 公式ドキュメント + Playwright | HTML→PPTX 変換         |
| `convert_html_cli.js` | 上記のラッパー                          | CLI インターフェース   |

### 結果

```
scripts/
├── convert_html.js       # HTML→PPTX 変換
├── convert_html_cli.js   # CLI ラッパー
├── convert_html_multi.js # 複数ファイル一括変換
...
## Step 6: 動作検証（テンプレート置換）

### 指示例

```
templates/ にある sample.pptx を使って
inventory → replace の流れを検証して
```

### 実行された作業

1. `python scripts/extract_shapes.py` でテキスト構造を抽出
2. `output_manifest/replacements.json` を作成（AI が自動生成）
3. `python scripts/apply_content.py` で置換実行
4. 成功

**実装のポイント**:

- python-pptx の公式ドキュメントを参考に独自実装
- 文字エンコーディングは UTF-8 を明示
- 色の取得時は `_SchemeColor` 型への対応を追加
