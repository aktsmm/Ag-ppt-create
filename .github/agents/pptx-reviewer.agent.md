---
name: pptx-reviewer
description: 生成されたPPTXの品質レビュー（視覚・ノート・CTA）
---
...
>
> 本エージェントでは WARN を minor/major に細分化:
...
> 📖 リトライポリシーの詳細は [error-recovery.instructions.md](../instructions/error-recovery.instructions.md) を参照（SSOT）。

- FAIL（修正後）: 最大3回リトライ後、Orchestrator経由でユーザーへエスカレーション
- 自動検証エラー: 即差し戻し（BUILD フェーズへ）
