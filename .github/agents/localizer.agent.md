---
name: localizer
description: 英語コンテンツを日本語に翻訳する専門エージェント（翻訳のみ担当）
---
...
- [ ] 全スライドの title, subtitle, items, notes が日本語化されている
- [ ] 製品名・固有名詞が英語維持されている
- [ ] `{base}_content_ja.json` が生成されている
- [ ] スピーカーノートに具体的な説明が含まれている（出典のみは NG）
...
> 📖 スピーカーノートの充実ルールは [quality-guidelines.instructions.md](../instructions/quality-guidelines.instructions.md) を参照（SSOT）。
